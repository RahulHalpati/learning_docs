# 04-1 · The aws provider against Floci

> **Level:** Intermediate · **Prerequisites:** [03-2 DynamoDB](../03_core_services/02_dynamodb.md), [OpenTofu course](../../opentofu_iac/)
> **Time:** 25 min · **Verified:** 2026-07-16 (OpenTofu v1.12.4 + LocalStack 3.8.1; runs unchanged on Floci)
...
**real `hashicorp/aws` provider** — just aimed at Floci. Your config becomes
production-ready by deleting a few lines.

---

## Point the aws provider at Floci
...
provider "aws" {
  region                      = "us-east-1"
  access_key                  = "test"       # ignored by Floci
  secret_key                  = "test"
  s3_use_path_style           = true         # Floci S3 quirk (02-1)
...
The `skip_*` flags exist because Floci has no real IAM/account/metadata to
...
The resources below are **ordinary AWS resources** — nothing Floci-specific:
...
queue_url   = "http://localhost:4566/000000000000/linkstash-events"
table_name  = "links"
```

Three real AWS resource types, created on Floci, in one command.

---

## `tflocal` — the shortcut

Floci ships **`tflocal`**, a wrapper that injects the endpoints/skip config for
you, so your `.tf` can be endpoint-free:

```bash
pip install terraform-local
tflocal init && tflocal apply     # same as tofu, but auto-points at Floci
```
...
- ✅ Use the **real `hashicorp/aws` provider** with an `endpoints` block + `skip_*`
  flags to target Floci.
- ✅ The resources are **ordinary AWS resources** — remove the Floci lines and
...
friends for Floci, when a real-AWS provider doesn't set them?

<details>
<summary>Answer</summary>

On startup the aws provider normally validates credentials (via STS), checks the
EC2 metadata endpoint, and looks up the account id. Floci has no real
...
**→ Next: [04-2 · The linkstash-cloud stack](02_the_capstone_stack.md)**
